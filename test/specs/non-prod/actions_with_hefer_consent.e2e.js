...
describe('Actions that require HEFER Consent @cdp @ci @dev @hefer', () => {
  describe('Given farmer is eligible for funding', () => {
    describe('When farmer goes through the land grants application', () => {
      ...
      it('Then the farmer adds an action to the land parcel', async () => {
        ...
        await expect(
          await ActionsPage.getHeferMessageForLivestockGrazing()
        ).toContain(
          'You must get a HEFER (opens in new tab) to do these actions on this land parcel.'
        )
